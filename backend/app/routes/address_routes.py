...
@address_routes.route("/add/", methods=["POST"], strict_slashes=False)
@auth_required
def create_address():
...
@address_routes.route("/", methods=["GET"], strict_slashes=False)
@auth_required
def fetch_addresses():
    return jsonify(get_addresses(g.user_id))

@address_routes.route("/<address_id>", methods=["DELETE"])
@address_routes.route("/<address_id>/", methods=["DELETE"], strict_slashes=False)
@auth_required
...
@address_routes.route("/default/<address_id>/", methods=["PUT"], strict_slashes=False)
@auth_required
...
@address_routes.route("/<address_id>/", methods=["PUT"], strict_slashes=False)
@auth_required
